...
struct RLConfig {
	...
	int D = 3;					// D = #features (bias + r/n + c/m)
};
...
struct QLearner {
	...
	void initialization(RLConfig &cfg) {
		C = cfg;
		C.alpha = C.alpha0;
		C.eps = C.eps0;
		C.tau = C.tau0;
		if (C.featureQA) {
			...
			Q = vector<vector<double> >(A, vector<double>(C.D, 0.0));
			for (auto &wa : Q)
				for (auto &vd : wa)
					vd = nd(rng);
		} else {
			// optimistic initialization
			Q = vector<vector<double>>(S, vector<double>(A, 1.0));
		}
	}

	// φ(s): [1, r/(N-1), c/(N-1)]
	vector<double> phi(int s) {
		auto [r, c] = coords(s, GRID_SIZE);
		return {1.0, double(r) / (GRID_SIZE - 1), double(c) / (GRID_SIZE - 1)};
	}
	...
	int choose(int s) {
		if (!C.useBoltzmann && ur(rng) < C.eps) {
			return uniform_int_distribution<>(0, A - 1)(rng);
		}
		if (!C.useBoltzmann && !C.featureQA) {
			// greedy
			auto &row = Q[s];
		...
		if (!C.useBoltzmann && C.featureQA) {
			...
			double sum = 0;
			for (int a = 0; a < A; a++) {
				ex[a] = exp(Qval(s, a) / max(C.tau, 1e-6));
				sum += ex[a];
			}
			double r = ur(rng) * sum;
			for (int a = 0; a < A; a++) {
	...
	void update(int s, int a, int sp, double r) {
		if (C.featureQA) {
			...
			auto f = phi(s);
			for (int d = 0; d < C.D; d++) {
				Q[a][d] += C.alpha * (target - pred) * f[d];
			}
		} else {
			double bestp = *max_element(Q[sp].begin(), Q[sp].end());
			Q[s][a] += C.alpha * (r + C.gamma * bestp - Q[s][a]);
		}
	}

	void decay() {
		C.eps = max(C.epsMin, C.eps * C.epsDecay);
		C.alpha = max(0.01, C.alpha * (1 - C.alphaDecay));
		C.tau = max(0.01, C.tau * (1 - C.tauDecay));
	}

	void save(int worldId) {
		json J;
		J["Q"] = Q;
		ofstream f(("world_" + to_string(worldId) + (C.featureQA ? "_feat.json" : "_tab.json")));
		f << J.dump(2);
	}

	void load(int worldId) {
		ifstream f(("world_" + to_string(worldId) + (C.featureQA ? "_feat.json" : "_tab.json")));
		if (!f) return;
		json J;
		f >> J;
